...
SELECT DISTINCT
	...
	CASE 
		...
		THEN ROUND( -- Horas_Evento_1 + Horas_Evento_2
			 ...
			 -- Horas evento 2
			 (COALESCE(acs.hours, 0) / (DATEDIFF(b.fechaFin, b.fechaInicio) + 1)) * DAY(b.fechaFin) * b.factor, 2) 
		ELSE
			ROUND( -- Horas_Evento_1 + Horas_Evento_2
			 ...
			 ((DATEDIFF(b.fechaFin, b.fechaInicio) + 1) - DAY(b.fechaFin)) +
			 -- Horas evento 2
			 (COALESCE(se.horas, 0) / (DATEDIFF(b.fechaFin, b.fechaInicio) + 1)) * DAY(b.fechaFin)* b.factor, 2) 
	END	AS Horas_a_pago,
    ...
    CASE 
		...
		THEN
			...
			((DATEDIFF(b.fechaFin, b.fechaInicio) + 1) - DAY(b.fechaFin)) * b.factor, 2) 
		ELSE
			ROUND((COALESCE(se.horas, 0) / (DATEDIFF(b.fechaFin, b.fechaInicio) + 1)) * 
			((DATEDIFF(b.fechaFin, b.fechaInicio) + 1) - DAY(b.fechaFin)) * b.factor, 2) 
	END AS Horas_Evento_1,
    ...
    CASE 
		WHEN se.horas IS NULL
		THEN
			ROUND((COALESCE(acs.hours, 0) / (DATEDIFF(b.fechaFin, b.fechaInicio) + 1)) * DAY(b.fechaFin) * b.factor, 2) 
		ELSE 
			ROUND((COALESCE(se.horas, 0) / (DATEDIFF(b.fechaFin, b.fechaInicio) + 1)) * DAY(b.fechaFin) * b.factor, 2)
	END	AS Horas_Evento2,
...
-- Carreras del bimestre
LEFT JOIN carreras c 
    ON c.bimestre_id = se.bimestre_id 
    and c.codigo_plan =  acs.code
